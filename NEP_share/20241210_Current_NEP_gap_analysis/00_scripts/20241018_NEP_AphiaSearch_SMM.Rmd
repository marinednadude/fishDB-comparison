...
```{r}
NEP_list_assembled_clean <- read_delim(here("NEP_list_assembled_clean.txt"), col_types = "c", na = c("","NA"))
...
duplicates_removed <- NEP_list_assembled_clean %>%
  group_by(across(-row_id)) %>%
  filter(n() > 1) %>%
  ungroup()

NEP_list_assembled_clean_dedup <- NEP_list_assembled_clean %>%
  distinct(across(-row_id))

#TODO Add some more 00s for padding (in case lists get larger than 99,999)
NEP_list_assembled_clean <- NEP_list_assembled_clean_dedup %>%
  mutate(row_id = str_c("R", str_pad(row_number(), width = 5, pad = "0"))) %>%
  dplyr::select(row_id, everything())

NEP_list_assembled_clean$Scientific.Name %>% unique() %>%  length()
```

# Export cleaned list
```{r}
write.table(NEP_list_assembled_clean, file='NEP_list_assembled_clean_rowIDs.txt', sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)

```

#Create new list that is unique on Scientific.Name (concats the barcode by ;)
```{r}
scientific_name_table <- NEP_list_assembled_clean %>%
...
write.table(scientific_name_table_NO_APHIAID, file='NEP_MANUALCHECK_NOAPHIAID.txt', sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)
...
```{r}
NEP_NEWaddAphiaID <- read_excel(here("nep_manualcheck_noaphiaid_matched.xlsx"), na = c("","NA"))
...
scientific_name_table_NO_APHIAID <- scientific_name_table_NO_APHIAID %>%
  dplyr::select(-AphiaID)
write.table(scientific_name_table_NO_APHIAID, file='NEP_MANUALCHECK2_NOAPHIAID.txt', sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)

```

# Import external WORMS search/manual no_aphiaID correction and merge
```{r}
NEP_NEWaddAphiaID <- read_excel(here("NEP_MANUALCHECK2_NOAPHIAID.xlsx"), na = c("","NA"))
NEP_NEWaddAphiaID <- NEP_NEWaddAphiaID %>% mutate(AphiaID = as.integer(AphiaID))
...
```{r}
WORMS_db <- read_delim(here("WoRMS_download_2024-08-01","taxon.txt"), col_types = "c", na = c("","NA"))
WORMS_identifier_db <- read_delim(here("WoRMS_download_2024-08-01","identifier.txt"), col_types = "c", na = c("","NA"))
...
#write.table(new_records_df, file='NEP_NEWRECORDSDF.txt', sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)
#write.table(merge_scientificName_WORMSdb, file='NEP_merge_scientificName_WORMSdb.txt', sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)
...
```{r}
NEP_list_assembled_clean_NOT_SEARCHING_NCBI <- left_join(remaining_records, NEP_list_assembled_clean, by = "Scientific.Name")

write.table(NEP_list_assembled_clean_NOT_SEARCHING_NCBI, file=here("export/NEP_list_assembled_clean_gtGenus_NOT_SEARCHING_NCBI.txt"), sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)
...
```{r}
NEP_list_assembled_clean_GENUS_NOT_SEARCHING_NCBI <- left_join(Genus_noNCBIid, NEP_list_assembled_clean, by = "Scientific.Name")

write.table(NEP_list_assembled_clean_GENUS_NOT_SEARCHING_NCBI, file=here("export/NEP_list_assembled_clean_GenusNoNCBI_NOT_SEARCHING_NCBI.txt"), sep='\t', quote=FALSE, row.names=FALSE, col.names = TRUE)
