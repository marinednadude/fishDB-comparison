...
as.data.frame(fasta) %>% as_tibble(rownames = "Accession") %>% 
  left_join(tax_e) %>% dplyr::rename(Sequence = x) -> fasta_df

fasta_df %>% 
  filter(., !is.na(taxonomic_path)) %>% 
  filter(., !str_detect(taxonomic_path,"NA;NA;NA;NA")) %>% 
  filter(., !str_detect(taxonomic_path,"Bacteria;")) %>%
  filter(., !str_detect(taxonomic_path,"Archaea;")) -> fasta_df

fasta_df %>% 
  group_by(Sequence) %>% 
  mutate(cluster = cur_group_id()) -> clusters

clusters %>% 
  ungroup() %>% 
dplyr::rename(Name = taxonomic_path) %>% 
    ...
    .x %>%
      ...
      mutate(Same_Species = n_distinct(unlist(across(starts_with('Species'), 
             ...
             Same_Family = n_distinct(unlist(across(starts_with('Family'), 
                                                    ~ as.character(.x)))) == 1,
             Same_Order = n_distinct(unlist(across(starts_with('Order'), 
                                                   ~ as.character(.x)))) == 1,
             Same_Class = n_distinct(unlist(across(starts_with('Class'), 
                                                   ~ as.character(.x)))) == 1,
